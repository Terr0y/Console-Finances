...
function financialAnalysis(data) {
  ...
  let previousMonthAmount = data[0][1];
  let totalChange = 0;
  let greatestIncrease = { date: '', amount: Number.MIN_SAFE_INTEGER };
  let greatestDecrease = { date: '', amount: Number.MAX_SAFE_INTEGER };
  ...
  data.forEach((month, index) => {
    ...
    if (index !== 0) {
      let change = month[1] - previousMonthAmount;
      totalChange += change;

      // The greatest increase in Profit/Losses (date and amount) over the entire period.

      if (change > greatestIncrease.amount) {
        greatestIncrease.date = month[0];
      ...
      if (change < greatestDecrease.amount) {
        greatestDecrease.date = month[0];
        greatestDecrease.amount = change;
      }

      previousMonthAmount = month[1];
    }
  });
